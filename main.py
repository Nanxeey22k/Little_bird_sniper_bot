...
def main():
    ...
    async def run_all():
        # Health server starts first so Railway sees the port immediately
        await run_health_server()
        await telegram_app.initialize()

        # 409 Conflict fix:
        # Railway starts the new container before killing the old one.
        # We wait 10s for the old instance to fully die, then clear any
        # lingering polling session before starting our own.
        logger.info("Waiting 10s for previous instance to shut down...")
        await asyncio.sleep(10)

        # Retry delete_webhook up to 5 times with increasing backoff
        for attempt in range(1, 6):
            try:
                await telegram_app.bot.delete_webhook(drop_pending_updates=True)
                logger.info("Webhook cleared - 409 prevention OK")
                break
            except Exception as e:
                logger.warning(f"Webhook clear attempt {attempt}/5: {e}")
                if attempt < 5:
                    await asyncio.sleep(5 * attempt)  # 5s, 10s, 15s, 20s
